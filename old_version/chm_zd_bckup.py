...
class DlgMain(QDialog, Ui_dlg_chm):
    ...
    def evt_btn_calculate_clicked(self):
        ...
        if not os.path.exists(fn) or not os.path.isdir(fc):
            ...
            # PATHS TO FILES
            dtm_fn = fc + r"\DTM_{}.tif".format(os.path.basename(fn)[:-4])
            dsm_fn = fc + r"\DSM_{}.tif".format(os.path.basename(fn)[:-4])
            chm_fn = fc + r"\CHM_{}.tif".format(os.path.basename(fn)[:-4])
            seg_tif_fn = fc + r"\SEGMENTS_{}.tif".format(os.path.basename(fn)[:-4])
            segments_fn = fc + r"\SEGMENTS_{}.shp".format(os.path.basename(fn)[:-4])
            csv_fn = fc + r"\ZSTATS_{}.csv".format(os.path.basename(fn)[:-4])
